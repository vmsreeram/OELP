...
ipcMain.on('load_csv',()=>{
  ...
  csvmainWindow.loadFile(path.join(__dirname, 'csv_reader.html'));
  csvmainWindow.webContents.openDevTools();
  csvmainWindow.on('closed', function () {
    csvmainWindow = null;
  });
});
...
ipcMain.handle('open-csv-dialog', async (event) => {
  const result = await dialog.showOpenDialog(csvmainWindow, {
    properties: ['openFile'],
    filters: [
  ...
  if (!result.canceled && result.filePaths.length > 0) {
    ...
    stream.pipe(csv())
      ...
      .on('end', () => {
        ...
        console.log(csvData)
        csvmainWindow.webContents.send('csv-data', csvData);
      })
      .on('error', (err) => {
